...
def process_channel_data(channels: List[Tuple[str, ...]]) -> Dict[str, List[str]]:
    ...
    with open(os.getcwd() +'/output/sctv.txt', 'w', encoding='utf-8') as ftxt, \
            ...
            for channel in channels:
                if condition(channel[1]):
                    ...
                    rtspUrl = channel[5].replace(
                        "rrsip=.+?\CEND", f'playseek=${(b)yyyyMMddHHmmss}-${(e)yyyyMMddHHmmss') if channel[4] == '1' else None
        ...
        write_channel('四川频道', lambda name: any(x in name for x in ['SCTV', '四川', 'CDTV', '成都', '熊猫', '峨眉']))
