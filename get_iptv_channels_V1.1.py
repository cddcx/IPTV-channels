...
def process_channel_data(channels: List[Tuple[str, ...]]) -> Dict[str, List[str]]:
    ...
    # 过滤无效频道
    channels = [ch for ch in channels if "画中画" not in ch[1] and "单音轨" not in ch[1] and "直播室" not in ch[1] and "热门" not in ch[1] and "爱" not in ch[1] and "92" not in ch[1]]
            ...
            for channel in channels:
                if condition(channel[1]):
                    ...
                    rtspUrl = channel[6].replace(
                        "rtsp://", f'rtsp://')+'?playseek=${(b)yyyyMMddHHmmss}-${(e)yyyyMMddHHmmss}' if channel[4] == '1' else None
